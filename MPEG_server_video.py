...
# Function to record video and send to the receiving Pi (now as a separate process)
...
picam2.start_encoder(stream_encoder, FileOutput(output))
picam2.start()

# Start the server and video sending in separate processes
if __name__ == "__main__":
    # Start video sending in a separate process
    video_process = Process(target=record_and_send_video)
    video_process.start()
    ...
    try:
        ...
        video_process.terminate()  # Ensure the video process is stopped when the app exits
